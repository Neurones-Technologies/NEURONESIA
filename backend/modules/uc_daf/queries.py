...
# ── Contrôles de qualité de saisie (volet formation) ────────────────────────

async def fetch_controles_qualite() -> dict[str, dict]:
    """Compteurs de défaut de saisie, mesurés — un compteur par module de formation.

    Chaque entrée porte le nombre de lignes en défaut ET l'assiette : « 1 507 »
    ne veut rien dire, « 1 507 clients sur 1 508 » se comprend et se corrige.
    Les requêtes sont des agrégats indexés, pas des lectures complètes : elles
    peuvent rester dans la boucle.
    """
    return {
        "echeance_incoherente": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM invoices WHERE due_date IS NULL OR due_date < invoice_date"
            ),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM invoices"),
        },
        "reglement_non_rattache": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM invoices WHERE status = 'paid' AND payment_date IS NULL"
            ),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM invoices WHERE status = 'paid'"),
        },
        "facture_fournisseur_absente": {
            "nb_defaut": await _scalaire("SELECT COUNT(*) FROM purchase_orders"),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM purchase_orders"),
            # Assiette = les achats engagés : tant qu'aucune facture fournisseur
            # n'existe, chacun d'eux est une dette future non échéancée.
            "nb_factures_fournisseurs": await _scalaire("SELECT COUNT(*) FROM supplier_invoices"),
        },
        "achat_sans_dossier": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM purchase_orders WHERE dossier_id IS NULL OR TRIM(dossier_id) = ''"
            ),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM purchase_orders"),
        },
        "dossier_sans_depense": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM dossiers WHERE depense_definitive IS NULL OR depense_definitive = 0"
            ),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM dossiers"),
        },
        "client_sans_secteur": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM clients WHERE sector IS NULL OR TRIM(sector) = ''"
            ),
            "nb_total": await _scalaire("SELECT COUNT(*) FROM clients"),
        },
        "document_en_devise": {
            "nb_defaut": await _scalaire(
                "SELECT (SELECT COUNT(*) FROM purchase_orders WHERE currency != 'XOF') "
                "     + (SELECT COUNT(*) FROM invoices WHERE currency != 'XOF')"
            ),
            "nb_total": await _scalaire(
                "SELECT (SELECT COUNT(*) FROM purchase_orders) + (SELECT COUNT(*) FROM invoices)"
            ),
        },
        "facture_sans_client": {
            "nb_defaut": await _scalaire(
                "SELECT COUNT(*) FROM invoices i LEFT JOIN clients c ON c.client_id = i.client_id "
