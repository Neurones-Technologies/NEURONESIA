...
# ── Volet transverse — Formation des utilisateurs ────────────────────────────
#
# Demande explicite du DAF : « formation des utilisateurs aux bons usages de
# l'outil, afin d'éviter tout impact négatif (erreur de saisie, corruption) sur
# les données ».
#
# Le contenu pédagogique ci-dessous est POSÉ (il n'y a pas de référentiel de
# formation dans le système), mais chaque module est rattaché à un contrôle de
# qualité MESURÉ sur les données réelles (`code_controle`) : la formation est
# ainsi priorisée par ce qui est réellement mal saisi, et non par ce qu'on
# imagine mal saisi. Les compteurs de défaut viennent de `formation.py`.
MODULES_FORMATION = [
    {
        "code": "echeances",
        "titre": "Saisir une échéance de facture qui veut dire quelque chose",
        "public": "Comptabilité clients, ADV",
        "duree_min": 45,
        "code_controle": "echeance_incoherente",
        "objectif": "Toute la trésorerie prévisionnelle repose sur la date d'échéance : une échéance "
                    "fausse déplace un encaissement de plusieurs semaines sur le calendrier.",
        "points": [
            "L'échéance se déduit du délai négocié au contrat, jamais de la date du jour.",
            "Une échéance antérieure à la date de facture est toujours une erreur de saisie.",
            "Un avoir ne se saisit pas comme une facture à échéance dépassée.",
        ],
        "risque_si_absent": "Créances vues comme échues alors qu'elles ne le sont pas — relances "
                            "envoyées à tort et prévision d'encaissement décalée.",
    },
    {
        "code": "reglements",
        "titre": "Rattacher un règlement à sa facture",
        "public": "Comptabilité, trésorerie",
        "duree_min": 60,
        "code_controle": "reglement_non_rattache",
        "objectif": "Le DSO réel et le comportement de paiement de chaque client se calculent sur la "
                    "date de règlement rattachée. Sans elle, il ne reste qu'une approximation.",
        "points": [
            "Un encaissement se rattache à la facture qu'il solde, pas au compte client globalement.",
            "Un règlement partiel se saisit partiellement : le reste dû doit rester visible.",
            "Une facture soldée sans date de règlement rend le délai de paiement incalculable.",
        ],
        "risque_si_absent": "DSO faux, mauvais payeurs invisibles, prévision d'encaissement aveugle.",
    },
    {
        "code": "fournisseurs",
        "titre": "Enregistrer une facture fournisseur et son échéance",
        "public": "Comptabilité fournisseurs, achats",
        "duree_min": 60,
        "code_controle": "facture_fournisseur_absente",
        "objectif": "Le DPO et la moitié du plan de trésorerie dépendent de factures fournisseurs "
                    "qui, aujourd'hui, n'existent pas dans le système.",
        "points": [
            "Une commande d'achat n'est pas une dette : c'est la facture reçue qui l'est.",
            "Le délai de paiement négocié se saisit sur la fiche fournisseur, une fois pour toutes.",
            "Un règlement fournisseur se rattache à sa facture, comme côté client.",
        ],
        "risque_si_absent": "Décaissements non échéancés : le calendrier d'atterrissage reste une "
                            "projection au lieu d'être un plan.",
    },
    {
        "code": "achats_dossier",
        "titre": "Rattacher un achat au dossier qu'il sert",
        "public": "Achats, chefs de projet",
        "duree_min": 30,
        "code_controle": "achat_sans_dossier",
        "objectif": "La marge d'un dossier n'est fiable que si les achats qui l'ont servi lui sont "
                    "rattachés. Sans rattachement, la dépense existe mais n'est imputée à rien.",
        "points": [
            "Le dossier se renseigne à la création de la commande d'achat, pas après livraison.",
            "Un achat mutualisé se ventile plutôt que d'être affecté au premier dossier venu.",
            "Un achat sans dossier tombe dans les frais généraux et fausse la marge du projet.",
        ],
        "risque_si_absent": "Marge brute sous-estimée sur les dossiers imputés, surestimée ailleurs.",
    },
    {
        "code": "cloture_dossier",
        "titre": "Clôturer un dossier : dépense définitive et CA définitif",
        "public": "Contrôle de gestion, chefs de projet",
        "duree_min": 45,
        "code_controle": "dossier_sans_depense",
        "objectif": "La marge brute réalisée à date se calcule sur les dossiers dont la dépense "
                    "définitive est imputée. Les autres ne sont pas mesurables.",
        "points": [
            "Un dossier livré doit porter sa dépense définitive, même si une facture reste à venir.",
            "Le provisoire n'a de valeur que tant que le définitif n'existe pas.",
            "Un dossier à marge de 100 % est presque toujours un dossier dont la dépense manque.",
        ],
        "risque_si_absent": "Marge brute flatteuse et non exploitable : c'est le cas de l'exercice en "
                            "cours aujourd'hui.",
    },
    {
        "code": "referentiel_client",
        "titre": "Tenir le référentiel client (secteur, solvabilité, contact)",
        "public": "ADV, commerce",
        "duree_min": 30,
        "code_controle": "client_sans_secteur",
        "objectif": "L'analyse du risque client par secteur et le scoring de solvabilité ne "
                    "fonctionnent que sur un référentiel renseigné.",
        "points": [
            "Le secteur d'activité se choisit dans la nomenclature validée, pas en texte libre.",
            "Un même client ne doit pas exister sous deux fiches : le doublon casse tous les cumuls.",
            "Le contact de recouvrement est une information financière, pas seulement commerciale.",
        ],
        "risque_si_absent": "Aucune lecture sectorielle du risque client ; relances adressées au "
                            "mauvais interlocuteur.",
    },
    {
        "code": "devises",
        "titre": "Saisir un document en devise",
        "public": "Achats, comptabilité",
        "duree_min": 30,
        "code_controle": "document_en_devise",
        "objectif": "Un tiers des achats sont libellés en devise. Le montant est converti par l'ERP, "
                    "le taux appliqué ne l'est pas dans le miroir.",
        "points": [
            "La devise du document se saisit avec le taux de la date d'engagement.",
            "Un montant en devise ne se compare pas à un montant en FCFA sans conversion explicite.",
            "Un écart de change constaté au règlement se comptabilise, il ne se lisse pas.",
        ],
        "risque_si_absent": "Charges cumulées à un taux implicite : le top des charges peut se "
                            "classer différemment selon le taux retenu.",
    },
]

REGLES_OR = [
    "Une donnée fausse saisie vite coûte plus cher qu'une donnée juste saisie lentement : tout ce "
    "qui est affiché dans ce cockpit est recalculé à partir de la saisie, sans filet.",
    "Aucune correction ne se fait dans le cockpit : il lit le miroir de l'ERP, il ne l'écrit pas. "
    "Une valeur fausse se corrige dans Odoo, à la source.",
    "Un champ qu'on ne sait pas remplir se laisse vide plutôt que rempli au hasard : un champ vide "
    "est visible dans les contrôles de qualité, une valeur inventée ne l'est pas.",
    "Un indicateur qui paraît faux ne se contourne pas : il se signale. Neuf fois sur dix, c'est la "
    "donnée qui est fausse, et la corriger profite à tout le monde.",
    "Les écrans marqués « données statiques » ne se citent jamais en réunion comme un chiffre "
    "mesuré : ils montrent la forme de l'indicateur, pas la réalité de l'entreprise.",
]

NOTE_FORMATION = (
    "Le contenu pédagogique de cet écran est posé ; les compteurs de défaut, eux, sont MESURÉS sur "
    "les données réelles du miroir. C'est ce qui permet de prioriser la formation par ce qui est "
    "réellement mal saisi, et de vérifier après coup que la formation a produit un effet."
)
