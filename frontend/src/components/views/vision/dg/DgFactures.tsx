      ...
      {relation && balance && detteFournisseur && cycleCash ? (
        <Bento>
          <StatTile
            ...
            value={formatFcfa(relation.dso.encours_echu_xof)}
            unit="FCFA"
            reading={
            ...
            detail={{
              ...
              body: [
                `Sur ${formatFcfa(relation.dso.encours_xof)} FCFA d'encours client ouvert, ${formatFcfa(relation.dso.encours_echu_xof)} FCFA ont dépassé leur échéance, portés par ${formatNumber(nbEchuesClient)} factures.`,
                tranche90Client
                  ? `${formatFcfa(tranche90Client.montant_xof)} FCFA ont plus de 90 jours de retard : au-delà de ce seuil, le recouvrement amiable a rarement encore prise — c'est la tranche qui détermine le besoin de provision.`
                  : "",
                balance.note,
              ].filter(Boolean),
              kv: [
                ["Encours ouvert", `${formatFcfa(relation.dso.encours_xof)} FCFA`],
                ["Dont échu", `${formatFcfa(relation.dso.encours_echu_xof)} FCFA`],
                [
                  "Dont plus de 90 jours",
                  tranche90Client ? `${formatFcfa(tranche90Client.montant_xof)} FCFA` : "—",
                ],
          ...
          <StatTile
            ...
            value={formatFcfa(detteFournisseur.dette_echue_xof)}
            unit="FCFA"
            reading={
            ...
            detail={{
              ...
              body: [
                `${formatFcfa(detteFournisseur.dette_xof)} FCFA de dette fournisseur ouverte, dont ${formatFcfa(detteFournisseur.dette_echue_xof)} FCFA au-delà de l'échéance.`,
                detteFournisseur.note,
              ],
              kv: [
                ["Dette ouverte", `${formatFcfa(detteFournisseur.dette_xof)} FCFA`],
                ["Dont échue", `${formatFcfa(detteFournisseur.dette_echue_xof)} FCFA`],
          ...
          >
            ...
            {tranche90Client && tranche90Client.part_pct > 50 && (
              <Note accent style={{ marginTop: 14 }}>
                ...
                retard, soit {formatFcfa(tranche90Client.montant_xof)} FCFA sur{" "}
          ...
          <Tile
            ...
            kick={sourceKick(detteFournisseur.source, `${formatFcfa(detteFournisseur.dette_xof)} FCFA`)}
          ...
          {payeurs && payeurs.clients.length > 0 && (
            <Tile
              ...
              kick={`${formatNumber(payeurs.totaux.nb_clients_a_risque)} client(s) à risque · ${formatFcfa(payeurs.totaux.encours_echu_total_xof)} FCFA échus`}
            ...
            >
              <Bars
                ...
                rows={payeurs.clients.slice(0, 8).map((p) => ({
                  ...
                  value: `${formatFcfa(p.encours_echu_xof)}`,
