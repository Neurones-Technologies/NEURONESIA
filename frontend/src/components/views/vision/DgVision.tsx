...
export async function DgVision() {
  ...
  const [briefing, kpis, forecast, topClients, topOrders, variance, monthlyClients, pilotage, relation] =
    await Promise.all([
      getBriefing(),
      getKpis(year),
      getForecastPipelineWeighted(),
      ...
      getPilotageDg(year),
      getRelationCommerciale(undefined, 8),
    ]);
  ...
  // ── Factures (situation client / fournisseur) — dérivés ───────────────────
  // `relation` vient de /v1/daf/relation-commerciale : les MÊMES chiffres que
  // l'écran Relation commerciale du DAF et que l'élément de débrief
  // « Factures échues » (uc_daf.situation_factures) — jamais deux chiffrages.
  ...
  return (
    <>
      ...
      {/* Miroir de l'écran Relation commerciale du DAF, réduit à ce qui se
          décide au niveau DG : le stock échu des deux côtés. Même endpoint,
          mêmes chiffres — et le même socle que l'élément de débrief
          « Factures échues ». */}
      <Section
        id="factures"
        title="Factures"
